...
import { ShoppingCart, Search, ChevronLeft, ChevronRight, File } from 'lucide-react';
...
const PayableRow = React.memo(({ r, onOpen }) => (
  <tr>
    <td><span className="cell-link" onClick={() => onOpen(r.id)} onDoubleClick={() => onOpen(r.id)}>{r.doc_number}</span></td>
    <td>{fmtDate(r.doc_date)}</td>
    <td>{r.vendor_name || '—'}</td>
    <td style={{ color: r.pay_status === 'Overdue' ? '#C62828' : 'inherit' }}>{fmtDate(r.due_date)}</td>
    <td className="num">{fmt(r.bill_amount)}</td>
    <td className="num" style={{ color: '#2E7D32' }}>{fmt(r.paid_amount)}</td>
...
const summaryCards = [
  { l: 'Total Billed', key: 'bill_amount', bg: 'var(--brand-50)', c: 'var(--brand-dark)' },
  { l: 'Total Paid', key: 'paid_amount', bg: '#E8F5E9', c: '#2E7D32' },
  { l: 'Outstanding', key: 'balance_amount', bg: '#FFEBEE', c: '#C62828' },
  { l: 'Records', key: null, bg: '#E3F2FD', c: '#0D47A1' },
];
...
export default function AccountsPayable() {
  ...
  const [filters, setFilters] = usePersistedFilters('ap_filters', {
    vendor_id: '', from_date: '', to_date: '', status: '', overdue_only: false, search: '',
  });
  const reqId = useRef(0);

  useEffect(() => {
    api.get('/api/vendors?limit=500').then(r => setVendors(r.data || [])).catch(() => { });
  }, []);
  ...
  const load = useCallback(async (overrides) => {
    ...
    try {
      ...
      const ff = overrides || filters;
      if (ff.vendor_id) p.set('vendor_id', ff.vendor_id);
      ...
      const res = await api.get(`/api/reports/accounts-payable?${p}`);
      if (id === reqId.current) setData(res);
    } catch (err) { console.error("API Error in AccountsPayable:", err); alert("API Error: " + err.message); } finally { if (id === reqId.current) setLoading(false); }
  }, [filters, page]);
  ...
  const defaults = { vendor_id: '', from_date: '', to_date: '', status: '', overdue_only: false, search: '' };
  const hasActiveFilters = filters.vendor_id || filters.from_date || filters.to_date || filters.status || filters.overdue_only || filters.search;
  const handleClear = useCallback(() => { setPage(1); setFilters(defaults); load(defaults); }, [load]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const cards = useMemo(() => data ? summaryCards.map(s => ({
    ...s, v: s.key ? fmt(data.totals?.[s.key] || 0) : (data.total || 0),
  })) : [], [data]);
  ...
  return (
    <div style={{ padding: 20, height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }} className="animate-in">
      ...
      <div className="form-row" style={{ marginBottom: 16, background: 'var(--g50)', padding: 14, borderRadius: 10, border: '1px solid var(--g200)', flexWrap: 'wrap', flexShrink: 0 }}>
        <div className="fg">
          <label>Vendor</label>
          <SelectDropdown value={filters.vendor_id} onChange={e => f('vendor_id', e.target.value)}>
      ...
      {data && !loading && (
        <div style={{ display: 'grid', gridTemplateRows: 'auto 1fr auto auto', flex: 1, minHeight: 0 }}>
            ...
            <table className="dgrid" style={{ fontSize: 12, width: '100%', borderCollapse: 'collapse' }}>
              ...
              <thead style={{ position: 'sticky', top: 0, zIndex: 10, background: '#fff', boxShadow: '0 1px 2px rgba(0,0,0,0.1)' }}>
                <tr>
                  ...
                  <th style={{ width: 110 }} className="num">Bill Amt (₹)</th>
                  <th style={{ width: 110 }} className="num">Paid (₹)</th>
              ...
              <tbody>
                ...
                {pageRows.map((r, i) => (
                  <PayableRow key={r.id || i} r={r} onOpen={openPurchase} />
                ))}
              </tbody>
              <tfoot style={{ position: 'sticky', bottom: 0, zIndex: 10, boxShadow: '0 -1px 0 var(--g200)' }}>
                <tr style={{ fontWeight: 700, background: 'var(--g50)' }}>
                  ...
                  <td className="num" style={{ borderTop: 'none' }}>{fmt(data.totals?.bill_amount || 0)}</td>
                  <td className="num" style={{ color: '#2E7D32', borderTop: 'none' }}>{fmt(data.totals?.paid_amount || 0)}</td>
      ...
      <Modal open={!!selectedPurchaseId} onClose={() => setSelectedPurchaseId(null)} title={`Purchase Details`} icon={<File size={16} />} large>
        {loadingPurchase ? (
          ...
          <div style={{ padding: 16 }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
              <div>
                <p style={{ margin: '0 0 4px', fontSize: 11, color: 'var(--g500)', textTransform: 'uppercase', fontWeight: 600 }}>Vendor</p>
                <p style={{ margin: 0, fontWeight: 500 }}>{purchaseDetails.vendor_name || 'N/A'}</p>
              </div>
              <div>
                <p style={{ margin: '0 0 4px', fontSize: 11, color: 'var(--g500)', textTransform: 'uppercase', fontWeight: 600 }}>Purchase No</p>
                <p style={{ margin: 0, fontWeight: 500 }}>{purchaseDetails.doc_number}</p>
              </div>
              <div>
                <p style={{ margin: '0 0 4px', fontSize: 11, color: 'var(--g500)', textTransform: 'uppercase', fontWeight: 600 }}>Date</p>
                <p style={{ margin: 0, fontWeight: 500 }}>{fmtDate(purchaseDetails.doc_date)}</p>
              </div>
              <div>
                <p style={{ margin: '0 0 4px', fontSize: 11, color: 'var(--g500)', textTransform: 'uppercase', fontWeight: 600 }}>Grand Total</p>
                <p style={{ margin: 0, fontWeight: 600, color: 'var(--brand-dark)', fontSize: 16 }}>{fmt(purchaseDetails.grand_total)}</p>
              </div>
            </div>

            <h4 style={{ fontSize: 13, marginBottom: 8, paddingBottom: 4, borderBottom: '1px solid var(--g200)' }}>Lines</h4>
            {purchaseDetails.lines && purchaseDetails.lines.length > 0 ? (
          ...
          <div style={{ padding: 40, textAlign: 'center', color: 'var(--red)' }}>Failed to load purchase details.</div>
        )}
      </Modal>
    </div>
  );
