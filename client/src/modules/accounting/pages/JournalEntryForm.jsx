...
export default function JournalEntryForm() {
  ...
  const moreItems = readOnly ? [
    ...
    // Accounting Phase 0: a DRAFT may be deleted only when it is genuinely
    // manual. A draft carrying source_type was generated by another module
    // (payment, TDS, depreciation…) and is refused by the server with 409
    // ACCOUNTING_TRANSACTION_POSTED — offering the button here would only
    // produce a failed destructive action. Safe manual drafts keep Delete.
    viewData?.status === 'draft' && canEdit() && !viewData?.source_type && {
