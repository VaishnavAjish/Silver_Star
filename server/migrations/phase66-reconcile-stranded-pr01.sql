...
BEGIN
  ...
  FOR v_rec IN SELECT * FROM stranded_candidates LOOP
      ...
      IF (SELECT MAX(l.id) FROM lot_process_issues i JOIN inventory l ON l.id = i.lot_id JOIN items it ON it.id = l.item_id WHERE i.machine_process_id = v_rec.machine_process_id AND it.category = 'growth_run') <> v_rec.growth_inventory_id THEN
